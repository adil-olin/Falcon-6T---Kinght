...
{   
    ...
    if(continue_timer--<=0){
        ...
        //isbossnull is true if there is no boss
        if(isbossnull)
        {
    	    spawnenemy();
        }
        //boss will spawn after one minute in every level
        if(--boss_timer<=0)
        {
    ...
    //if player died the score will show after some delay
...
void initstage(void)
{
    ...
    r.x=210;
    //if it is new game then resetting the stage , else contiue the previous game
    if(isstarted==false)
    {
        resetStage();
    }
